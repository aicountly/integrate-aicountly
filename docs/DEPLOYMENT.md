...
| --- | --- | --- | --- |
| Deploy to cPanel Production | `web/dist/` then `server-php/` | `<remote root>/` and `<remote root>/api/` | https://integrate.aicountly.com (+ `/api`) |
| Deploy to cPanel Sandbox | `web/dist/` then `server-php/` | `<remote root>/` and `<remote root>/api/` | https://integrate.gh.aicountly.com (+ `/api`) |
...
`https://integrate.aicountly.com/api/.env` unless Apache is told otherwise.
