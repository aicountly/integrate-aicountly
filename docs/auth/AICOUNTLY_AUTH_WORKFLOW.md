# AICOUNTLY auth workflow (Integrate)

How Integrate signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
1. User opens `integrate.aicountly.com` (or `integrate.gh.aicountly.com`).
2. No `auth_token` → redirect to
   `{portal}/login/authentication_jump/integrate?returnUrl={origin}/auth/callback`.
...
| Integrate host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `integrate.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `integrate.aicountly.com/api` |
| `integrate.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `integrate.gh.aicountly.com/api` |
...
`integrate` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://integrate.gh.aicountly.com/api/health
...
curl -i -X POST https://integrate.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://integrate.gh.aicountly.com/api/global/login   # expect 404
```
